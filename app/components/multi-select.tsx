...
const MultipleSelector = React.forwardRef<MultipleSelectorRef, MultipleSelectorProps>(
  (
    ...
    useEffect(() => {
      ...
      const filtered = arrayOptions.filter((opt) => opt.label.toLowerCase().includes(inputValue.toLowerCase()));
      setOptions(transToGroupOption(filtered, groupBy));
    }, [arrayOptions, groupBy, inputValue, onSearch]);
    ...
    useEffect(() => {
      ...
      const exec = async () => {
        ...
        if (debouncedSearchTerm && debouncedSearchTerm.length >= 200) {
          await doSearch();
        }
      };
    ...
    const commandFilter = React.useCallback(() => {
      ...
      // Only apply filtering when input has 2 or more characters
      if (creatable || inputValue.length < 200) {
        return () => 1; // Return 1 to show all items
      }

      return (value: string, search: string) => {
        return value.toLowerCase().includes(search.toLowerCase()) ? 1 : -1;
      };
    }, [creatable, commandProps?.filter, inputValue.length]);
    ...
    return (
      ...
      >
        <div
          className={cn(
            'min-h-10 rounded-md border border-input text-sm',
            {
              ...
              'cursor-pointer': !disabled && selected.length !== 0,
            },
            className
          )}
          onClick={() => {
        ...
        <div className="relative">
          {open && (
            <CommandList
              className="absolute top-1 z-10 w-full max-h-[150px] overflow-y-auto rounded-md border bg-popover text-popover-foreground shadow-md outline-none animate-in"
              onMouseLeave={() => {
